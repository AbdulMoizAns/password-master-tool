...
export default function HibpCheck({ password }) {
  ...
  if (loading) { Icon = Loader2; text = 'Querying k-anonymity endpoint...'; color = '#00c8ff' }
    ...
    if (result.breached) { Icon = ShieldAlert; color = '#ff5050'; text = `⚠ BREACHED — seen ${result.count.toLocaleString()} times` }
    else if (result.error) { Icon = Shield; color = '#ffa000'; text = `Error: ${result.error}` }
  ...
  return (
    <div className="no-select border border-[var(--neon-glow)] rounded p-3 bg-black/40">
      <div className="flex items-center justify-between mb-2">
        <div className="text-[10px] tracking-widest text-[rgba(0,255,65,0.4)]">HIBP K-ANONYMITY CHECK</div>
        <button
          onClick={run}
          disabled={!password || loading}
          className="text-[10px] px-2 py-1 border border-[var(--neon-glow)] text-[var(--neon)] rounded hover:bg-[rgba(0,255,65,0.1)] disabled:opacity-30"
        >
          CHECK
        </button>
      </div>
