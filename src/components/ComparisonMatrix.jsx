...
function MetricRow({ label, values, colors }) {
  return (
    <div className="grid grid-cols-3 gap-2 mb-1">
      ...
      {values.map((v, i) => (
        <div key={i} className="text-xs font-bold text-center" style={{ color: colors?.[i] || 'var(--data-primary)', textShadow: '0 0 2px rgba(0,230,53,0.2)' }}>{v}</div>
      ))}
    </div>
  )
}
...
function StrengthBars({ values }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2 mb-2">
      {values.map((v, i) => {
        const pct = (v / max) * 100
        const c = v > 80 ? '#00FF41' : v > 60 ? '#00c8ff' : v > 40 ? '#FFD700' : '#FF3131'
        return (
          <div key={i} className="h-4 bg-black/60 rounded overflow-hidden border border-[var(--border-subtle)]">
...
export default function ComparisonMatrix() {
  ...
  const entColors = metrics.map(m => m.entropy > 80 ? '#00FF41' : m.entropy > 60 ? '#00c8ff' : m.entropy > 40 ? '#FFD700' : '#FF3131')

  return (
    <section className="no-select bento-panel">
      <div className="bento-panel-header">
        <h2 className="bento-panel-title">[ comparison.matrix ]</h2>
        <div className="bento-panel-badge">3-WAY ANALYSIS</div>
      </div>
        ...
        {SLOTS.map(i => (
          <div key={i} className="relative">
            <input
              ...
              placeholder={`PWD ${i + 1}`}
              className="input-field pr-6 allow-select"
            />
            {pwds[i] && (
              <button onClick={() => { const next = [...pwds]; next[i] = ''; setPwds(next) }} className="absolute right-1 top-1/2 -translate-y-1/2 text-muted hover:text-[var(--accent)]">
                <X size={12} />
              </button>
            )}
          </div>
      ...
      <div className="metric-cell">
        <div className="metric-label mb-2">COMPARISON METRICS</div>
          ...
          {metrics.map((m, i) => {
            ...
            return (
              <div key={i} className="text-[10px] flex items-start gap-1">
                <span className="neon-text-sm font-bold shrink-0 w-6">P{i + 1}:</span>
                {!pwds[i] ? (
                  <span className="text-muted">empty</span>
                ) : m.top20 ? (
                  <span className="text-[var(--danger)] flex items-center gap-1"><AlertTriangle size={10} /> TOP-20 BREACHED</span>
                ) : hasIssue ? (
                  <span className="text-[var(--warning)] flex items-center gap-1"><AlertTriangle size={10} /> {m.advice.slice(0, 2).join(', ')}</span>
                ) : (
                  <span className="neon-text-sm flex items-center gap-1"><CheckCircle size={10} /> Clean</span>
                )}
              </div>
            )
          })}
